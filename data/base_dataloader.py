...
class BaseDataLoader(ABC):
    ...
    def _load_data(self):
        if self.federated_config.rank == -1:
            ...
            self._load_local_algorithm_data()
    ...
    def _load_federated_data_on_server(self):
        if os.path.isfile(self.cached_data_file):
            ...
            train_features_dict, valid_features_dict, test_features_dict, \
                ...
                train_examples_num_dict, valid_examples_num_dict, test_examples_num_dict, train_num, valid_num, test_num \
                = pickle_read(self.cached_data_file)
            # server doesn't use each client's train & test dataset
            del train_features_dict, valid_features_dict, test_features_dict
        ...
        # self.train_loader = self.build_dataloader(train_fedtures_all, "train")
    ...
    def _load_federated_data_on_client(self):
        ...
        if self.federated_config.do_mimic:
            ...
            train_features_dict, valid_features_dict, test_features_dict, \
                ...
                train_examples_num_dict, valid_examples_num_dict, test_examples_num_dict, train_num, valid_num, test_num \
                = pickle_read(self.cached_data_file)
            del train_fedtures_all, valid_fedtures_all, test_fedtures_all
    ...
    def _load_centralized_data(self):
        ...
        train_dataloader_dict[-1] = self.build_dataloader(train_fedtures_all, "train")
        valid_dataloader_dict[-1] = self.build_dataloader(valid_fedtures_all, "valid")
        test_dataloader_dict[-1] = self.build_dataloader(test_fedtures_all, "test")
        for idx in self.clients_list:
            test_dataloader_dict[idx] = self.build_dataloader(test_features_dict[idx], "test")  # for local test

        self.train_dataloader_dict = train_dataloader_dict
        self.valid_dataloader_dict = valid_dataloader_dict
        self.test_dataloader_dict = test_dataloader_dict

    def _load_local_algorithm_data(self):
        train_dataloader_dict, valid_dataloader_dict, test_dataloader_dict = {}, {}, {}
    ...
    def _build_registry(self):
        ...
        if "token_classification" in self.model_config.model_output_mode:
    ...
    def build_dataloader(self, features, mode="train"):
        ...
        if self.output_mode == "seq_regression" or 'multi' in self.output_mode:
            all_labels = torch.tensor([f.label for f in features], dtype=torch.float)
        elif self.output_mode == "seq_generation":
            all_labels = copy.deepcopy(all_input_ids)
        else:
    ...
    def cached_data_file(self):
        ...
        if self.federated_config.rank != -1:
            ...
            cached_file_name = f"models={self.model_config.model_type}_{prompt_flag}" \
                               f"seq={self.data_config.max_seq_length}_" \
                               f"{self.federated_config.fl_algorithm}"
