...
def get_partition_data(examples, num_classes, num_clients,
                       label_vocab, dir_alpha, partition,):
    if partition == "iid":
        targets = [idx for idx, _ in enumerate(examples)]
    else:
...
def convert_glue_to_device_pkl(args):
    ...
    if f"clients={args.clients_num}_alpha={args.alpha}" in partition_data and not args.overwrite:
        ...
        logger.critical(f"partition is {partition}")
        clients_partition_data = {"train": get_partition_data(
            ...
            label_vocab=label_list, dir_alpha=args.alpha, partition=partition
